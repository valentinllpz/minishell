...
/*   Updated: 2021/04/22 13:05:00 by ade-garr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_unwrite_line(t_shell *shell)
{
	...
	{
		if (((ft_strlen_safe(shell->line) - i) + 2) % shell->term.nb_col == 0
		&& shell->term.pos_x == shell->term.nb_col)
			tputs(shell->term.del_c, 1, ft_putchar);
		else if (((ft_strlen_safe(shell->line) - i) + 2) % shell->term.nb_col == 0
		&& shell->term.pos_x == 1)
		{
		...
		i++;
		shell->term.pos_x = (((ft_strlen_safe(shell->line) - i) + 2)
		% shell->term.nb_col) + 1;
	}
}
