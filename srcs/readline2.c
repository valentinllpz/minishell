...
/*   Updated: 2021/04/22 09:40:41 by ade-garr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_unwrite_line(t_shell *shell)
{
	...
	{
		if (((ft_strlen_safe(shell->line) - i) + 2) % shell->nb_col == 0
		&& shell->pos_x == shell->nb_col)
			tputs(shell->del_c, 1, ft_putchar);
		else if (((ft_strlen_safe(shell->line) - i) + 2) % shell->nb_col == 0
		&& shell->pos_x == 1)
		{
			tputs(shell->line_up, 1, ft_putchar);
			tputs(tgoto(shell->end_line, 0, shell->nb_col - 1),
			1, ft_putchar);
			tputs(shell->del_c, 1, ft_putchar);
		}
		else
		...
		i++;
		shell->pos_x = (((ft_strlen_safe(shell->line) - i) + 2)
		% shell->nb_col) + 1;
	}
}
